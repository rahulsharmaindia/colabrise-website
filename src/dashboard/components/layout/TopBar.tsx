...
export function TopBar({ onMenuToggle, leftSlot, rightSlot, searchValue = '', onSearchChange }: TopBarProps) {
  return (
    <header className="sticky top-0 z-40 h-14 flex items-center gap-4 border-b border-white/5 bg-dark-900/80 backdrop-blur-sm px-4 lg:px-6">
      {/* Mobile menu button */}
      <button
      ...
      <div className="flex items-center gap-3">
        <div className="relative w-64 hidden sm:block">
          ...
          <input
            ...
            className="w-full pl-9 pr-8 py-1.5 text-sm rounded-lg border border-white/10 bg-white/5 text-white placeholder:text-gray-500 focus:outline-none focus:ring-2 focus:ring-purple-500/40 focus:border-purple-400/60"
          />
          {searchValue && (
            <button
